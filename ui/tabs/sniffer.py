...
class SnifferTab(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # ── Header ───────────────────────────────────────────────────
        header = ctk.CTkFrame(self, fg_color=theme.BG_PANEL, corner_radius=10)
        header.grid(row=0, column=0, columnspan=2, padx=12, pady=(12, 6),
                    sticky="ew")
        header.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(
            header, text="Sniffer / Matching",
        ...
        self._status_label.grid(row=0, column=1, padx=8, pady=10, sticky="w")

        self._btn_start = ctk.CTkButton(
            header, text="Start Sniffing",
        ...
        self._btn_start.grid(row=0, column=2, padx=4, pady=10)

        self._btn_clear = ctk.CTkButton(
            header, text="Clear",
            font=theme.FONT_BODY,
            fg_color=theme.BG_CARD_ALT, hover_color=theme.BG_HOVER,
            width=72, height=32, corner_radius=8,
            command=self._clear_traffic,
        )
        self._btn_clear.grid(row=0, column=3, padx=4, pady=10)

        self._btn_save = ctk.CTkButton(
            header, text="Save Matching",
            font=theme.FONT_BODY,
            fg_color=theme.ACCENT_DIM, hover_color=theme.ACCENT_HOVER,
            text_color=theme.BG,
            width=120, height=32, corner_radius=8,
        ...
        self._dir_filter.grid(row=0, column=5, padx=(4, 12), pady=10)
        ...
        ctk.CTkLabel(
            right, text="Live Traffic",
            font=theme.FONT_BODY, text_color=theme.TEXT,
        ).grid(row=0, column=0, padx=10, pady=(10, 4), sticky="w")

        self._traffic_text = tk.Text(
            right, bg=theme.BG, fg=theme.TEXT,
            font=("Consolas", 9),
            insertbackground=theme.TEXT,
            selectbackground=theme.ACCENT_DIM,
            relief="flat", wrap="none",
        ...
        self._traffic_text.grid(
            row=1, column=0, padx=8, pady=(0, 8), sticky="nsew")

        # Configure text tags
        for widget in [self._traffic_text, self._match_text]:
            widget.tag_configure("c2s", foreground=theme.TEXT_BLUE)
            widget.tag_configure("s2c", foreground=theme.TEXT_GREEN)
            widget.tag_configure("code", foreground=theme.ACCENT)
            widget.tag_configure("new", foreground=theme.TEXT_YELLOW)
            widget.tag_configure("dim", foreground=theme.TEXT_DIM)

        # ── Stats bar ────────────────────────────────────────────────
        stats = ctk.CTkFrame(self, fg_color=theme.BG_PANEL, corner_radius=8)
        stats.grid(row=2, column=0, columnspan=2, padx=12, pady=(4, 10),
                   sticky="ew")
        self._stats_label = ctk.CTkLabel(
            stats, text="Packets: 0 | Codes: 0 | New: 0",
            font=theme.FONT_SMALL, text_color=theme.TEXT_DIM,
        )
    ...
    def _toggle_sniff(self):
        if self._sniffing:
            ...
            self._status_label.configure(
                text="Stopped", text_color=theme.TEXT_DIM)
            if self._on_stop:
                self._on_stop()
        else:
            self._sniffing = True
            self._btn_start.configure(
                text="Stop Sniffing",
                fg_color=theme.TEXT_RED,
                hover_color="#b91c1c",
                text_color="#ffffff",
            )
            self._status_label.configure(
                text="Sniffing...", text_color=theme.TEXT_GREEN)
            if self._on_start:
                self._on_start()
    ...
    def _save_clicked(self):
        if self._on_save:
            self._on_save()
        self._status_label.configure(
            text="Matching saved!", text_color=theme.TEXT_GREEN)
    ...
    def _update_stats(self):
        total_codes = int(
            self._match_text.index("end-1c").split(".")[0]) - 1
        self._stats_label.configure(
            text=f"Packets: {self._packet_count}  |  "
                 f"Codes: {total_codes}  |  "
                 f"New: {self._new_codes}")
